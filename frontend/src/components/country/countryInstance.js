...
export default class CountryInstance extends Component {
  ...
  render() {
    ...
    const {alternateNames, area, borders, callingCodes, capital, codes, 
      currencies, flag, languages, location, name, news, population, 
      region, regionalBlocs, sources, timezones} = data;
    ...
    return (
      <div className='App'>
        <header className='Case-header'>
          ...
          <h1 id='page-title'> {name} ({codes.alpha3Code})</h1>
          <div>
            <div id="title-div">
              <h2 id='subtitle'> Capital - {capital.name}</h2>
            </div>
            <img src={capital.img} alt={"Image of " + capital.name} id="capital-image" />
          </div>
